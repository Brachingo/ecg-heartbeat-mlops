...
def balancear_datos(df_train):
    ...
    # Submuestreo de la clase mayoritaria al 65% del total de minoritarias
    df_majority_reducido = resample(
        df_majority,
        replace=False,
        n_samples=int(len(df_minority) * 0.65),
        random_state=42,
    )

    df_balanceado = pd.concat([df_majority_reducido, df_minority])
